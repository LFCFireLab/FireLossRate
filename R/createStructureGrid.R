...
#' @return Grid of squares with edge cellSize, with count of associated structures
#' @examples 
#' grid <- createBuildingGrid <- function(structures, 100, "maximal")
#' @export
#' 
createStructureGrid <- function(structures,
  ...
  if(internalBuffer){
    ...
    g_grid$Sum_Count[c(!st_contains(st_buffered, g_grid, sparse = F))] <- 0
  }
